...
class Version20260817200000 extends WallabagMigration
{
    public function up(Schema $schema): void
    {
        ...
        $existing = $connection->fetchOne(
            'SELECT * FROM ' . $this->getTable('craue_config_setting') . " WHERE name = 'tts_enabled'"
        );
        ...
        $this->addSql('INSERT INTO ' . $this->getTable('craue_config_setting') . " (name, value, section) VALUES ('tts_enabled', 0, 'entry')");
        $this->addSql('INSERT INTO ' . $this->getTable('craue_config_setting') . " (name, value, section) VALUES ('tts_base_path', '/tts', 'entry')");
    }

    public function down(Schema $schema): void
    {
        $this->addSql('DELETE FROM ' . $this->getTable('craue_config_setting') . " WHERE name = 'tts_enabled';");
        $this->addSql('DELETE FROM ' . $this->getTable('craue_config_setting') . " WHERE name = 'tts_base_path';");
    }
}
